...
;with x_foreign_keys(base_tbl_id, base_tbl, base_column_id, base_column, dependant_tbl_id, dependant_tbl, dependant_column_id, dependant_column, dependant_nullable)
as
(
	select 
...
,x_foreign_joins_delete(base_tbl, dependant_tbl, join_script)
as
(
	select 
		base_tbl,
	...
	where dependant_nullable = 0
)
,x_foreign_joins_update(base_tbl, dependant_tbl, dependant_column, join_script)
as
(
	select 
		base_tbl,
...
(
	...
	join x_foreign_joins_delete on tbl= base_tbl
	where dependant_tbl <> base_tbl
		and tbl_path not like ('%|' + base_tbl + '|' + dependant_tbl + '|%')
		and rn_txt not like '%U%'
	union all
	select
		rn_txt, 
		rn_txt + '_U',
		dependant_tbl,
		'update ' + rn_txt + '_U' + ' set ' + dependant_column + ' = NULL FROM ' + dependant_tbl + ' as [' + rn_txt + '_U] join ' + base_tbl + ' as ['+rn_txt+'] ON ' 
			+ REPLACE(REPLACE(join_script,'###',rn_txt),'$$$',rn_txt + '_' + convert(varchar(max),row_number() over(order by dependant_tbl))) + ' ' 
			+ script_tail,
		null,
		tbl_path + dependant_tbl + '|'
	from delete_order do
		join x_foreign_joins_update on tbl = base_tbl
	where rn_txt not like '%U%'
)
select * 
from delete_order
